...
import os
from utils import MaskedSoftmaxCELossWithLS
...
class TransformerTrainerValidator:
    def __init__(self, seed):
        ...
        self.loss = MaskedSoftmaxCELossWithLS(eps=transformer_hyperparas.smoothing)
    ...
    def read_file(self, seed):
        ...
        checkpoint_dir = os.path.join(paths.transformer_record_path, str(seed))
        checkpoint_path = os.path.join(checkpoint_dir, 'checkpoint_dict')
    ...
    def train(self, seed):
        while self.epochs_finished < self.max_epochs:
            ...
            if False:
                # Diagnostic: count and inspect batches
                count = 0
                for batch in self.train_data_iter:
                    X, X_valid_len, Y, Y_valid_len = [x.to(self.device) for x in batch]
                    count += 1
                    if count == 1:  # print only the first batch
                        print("First batch shapes:")
                        print("X:", X.shape, "Y:", Y.shape)
                    # (you can comment this whole loop out after checking)
                    break  # just inspect one batch and exit

                print("Approx. batches per epoch:", count)
            ...
            for batch in self.train_data_iter:
                ...
                if False:
                    with torch.no_grad():
                        cos_sims = []
                        for z_syn, z_sem in zip(syn_outputs, sem_outputs):
                            # Flatten over sequence and batch dimensions
                            sim = torch.nn.functional.cosine_similarity(
                                z_syn.flatten(start_dim=1),
                                z_sem.flatten(start_dim=1),
                                dim=1
                            ).mean().item()
                            cos_sims.append(sim)
                        mean_cos = sum(cos_sims) / len(cos_sims)
                        print(f"[Diag] Layer-wise cosine sims: {['%.3f' % s for s in cos_sims]}  (mean={mean_cos:.3f})")
                ...
                l = loss_fused + transformer_hyperparas.alpha * loss_sem + transformer_hyperparas.beta * loss_syn
                l = l.sum() / Y.shape[0]
