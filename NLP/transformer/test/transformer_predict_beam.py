...
class Candidate:
    ...
    def dec_state_process(self, dec_state):
        self.dec_state = []  # reset stored state
        # 1. Encoder outputs
        self.dec_state.append(dec_state[0].clone().to(transformer_hyperparas.device))
        # 2. Encoder valid lens
        self.dec_state.append(dec_state[1].clone().to(transformer_hyperparas.device))
        # 3. Syntax cache
        cache_syn = []
        for i in range(transformer_hyperparas.num_layers):
            if dec_state[2][i] is not None:
                cache_syn.append(dec_state[2][i].clone().to(transformer_hyperparas.device))
            else:
                cache_syn.append(None)
        self.dec_state.append(cache_syn)
        # 4. Semantic cache
        cache_sem = []
        for i in range(transformer_hyperparas.num_layers):
            if dec_state[3][i] is not None:
                cache_sem.append(dec_state[3][i].clone().to(transformer_hyperparas.device))
            else:
                cache_sem.append(None)
        self.dec_state.append(cache_sem)
        # 5. Y_valid_len
        self.dec_state.append(dec_state[4])
...
def transformer_predict_beam(net, src_sentence, src_vocab, tgt_vocab, num_steps, device, save_attention_weights=False):
    ...
    while step < num_steps and len(complete_sent_list) < transformer_hyperparas.topk:
        ...
        if step == 0:
            temp_candidate_list = []
            Y_fused, syn_outputs, sem_outputs, dec_state = net.decoder(dec_X, dec_state)
            Y = Y_fused
            ...
            for candidate in selected_list:
                Y_fused, syn_outputs, sem_outputs, dec_state = net.decoder(candidate.last_element_tensor, candidate.dec_state)
                Y = Y_fused
                Y1 = softmax(Y).squeeze(1)
                Y1 = Y1[0].cpu().detach().numpy().tolist()
                topk_value_list = \
